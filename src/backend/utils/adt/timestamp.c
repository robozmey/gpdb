...
timestamp_in(PG_FUNCTION_ARGS)
{
	...
	if (dterr == 0)
		if (!DecodeDateTimeSafe(field, ftype, nf, &dtype, tm, &fsec, &tz, &dterr, fcinfo->context))
		 	PG_RETURN_NULL();
	if (dterr != 0)
		if (!DateTimeParseErrorSafe(dterr, str, "timestamp", fcinfo->context))
			PG_RETURN_NULL();
	...
	if (!AdjustTimestampForTypmodSafe(&result, typmod, fcinfo->context))
		PG_RETURN_NULL();
...
timestamp_recv(PG_FUNCTION_ARGS)
{
	...
	if (!AdjustTimestampForTypmodSafe(&timestamp, typmod, fcinfo->context))
		PG_RETURN_NULL();
								...
								int32 shift, Timestamp reg, Timestamp *result, Node *escontext)
{
	...
	for (safety = 64; ; safety--)
	{
		...
		if (!timestamp_offset_multiple_safe(reg, width, index, &low, escontext))
			return false;
		if (!timestamp_offset_multiple_safe(low, width, 1, &high, escontext))
			return false;
	...
	if (shift)
		if (!timestamp_offset_multiple_safe(reg, width, index + shift, &low, escontext))
			return false;
...
timestamp_interval_bound(PG_FUNCTION_ARGS)
{
	...
	if(!timestamp_interval_bound_common_safe(
								  ...
								  int32 shift, TimestampTz reg, TimestampTz *result, Node *escontext)
{
	...
	for (safety = 64; ; safety--)
	{
		...
		if(!timestamptz_offset_multiple_safe(reg, width, index, &low, escontext))
			return false;
		if(!timestamptz_offset_multiple_safe(low, width, 1, &high, escontext))
			return false;
	...
	/* If necessary, shift the interval. */
	if (shift)
		if (!timestamptz_offset_multiple_safe(reg, width, index + shift, &low, escontext))
			return false;
...
timestamptz_interval_bound(PG_FUNCTION_ARGS)
{
	...
	if (!timestamptz_interval_bound_common_safe(
				val, width, shift, reg, &result, fcinfo->context))
		PG_RETURN_NULL();
...
timestamptz_in(PG_FUNCTION_ARGS)
{
	...
	if (dterr == 0)
		if (!DecodeDateTimeSafe(field, ftype, nf, &dtype, tm, &fsec, &tz, &dterr, fcinfo->context))
			PG_RETURN_NULL();
	if (dterr != 0)
		if (!DateTimeParseErrorSafe(dterr, str, "timestamp with time zone", fcinfo->context))
			PG_RETURN_NULL();
...
parse_sane_timezone_safe(struct pg_tm * tm, text *zone, int *result, Node *escontext)
{
	...
	if (rt != 0)
	{
		...
		lowzone = downcase_truncate_identifier(tzname,
											   strlen(tzname),
											   false);
		if (!DecodeTimezoneAbbrevSafe(0, lowzone, &val, &tzp, &type, escontext))
			return false;
...
make_timestamptz(PG_FUNCTION_ARGS)
{
	...
	if (!timestamp2timestamptz_safe(result, &result_tz, fcinfo->context))
		PG_RETURN_NULL();
...
interval_in(PG_FUNCTION_ARGS)
{
	...
	if (dterr != 0)
	{
		...
		if (!DateTimeParseErrorSafe(dterr, str, "interval", fcinfo->context))
			PG_RETURN_NULL();
	}
	...
	if (!AdjustIntervalForTypmodSafe(result, typmod, fcinfo->context))
		PG_RETURN_NULL();
...
interval_recv(PG_FUNCTION_ARGS)
{
	...
	if (!AdjustIntervalForTypmodSafe(interval, typmod, fcinfo->context))
		PG_RETURN_NULL();
...
timestamp_eq_timestamptz(PG_FUNCTION_ARGS)
{
	...
	if (!timestamp2timestamptz_safe(timestampVal, &dt1, fcinfo->context))
		PG_RETURN_NULL();
...
timestamptz_eq_timestamp(PG_FUNCTION_ARGS)
{
	...
	if (!timestamp2timestamptz_safe(timestampVal, &dt2, fcinfo->context))
		PG_RETURN_NULL();
...
timestamp_offset_multiple_safe(Timestamp base, Interval *unit, int64 mul, Timestamp *result, Node *escontext)
{
	...
	if (!timestamp_offset_internal_safe(base, &span, result, escontext))
		return false;
...
timestamptz_offset_multiple_safe(TimestampTz base, Interval *unit, int64 mul, TimestampTz *result, Node *escontext)
{
	...
	if (!timestamptz_offset_internal_safe(base, &span, result, escontext))
		return false;
...
timestamp_pl_interval(PG_FUNCTION_ARGS)
{
	...
	if (!timestamp_offset_internal_safe(timestamp, span, &result, fcinfo->context))
		PG_RETURN_NULL();
...
timestamp_mi_interval(PG_FUNCTION_ARGS)
{
	...
	if (!timestamp_offset_internal_safe(timestamp, &tspan, &result, fcinfo->context))
		PG_RETURN_NULL();
...
timestamptz_pl_interval(PG_FUNCTION_ARGS)
{
	...
	if (!timestamptz_offset_internal_safe(timestamp, span, &result, fcinfo->context))
		PG_RETURN_NULL();
...
timestamptz_mi_interval(PG_FUNCTION_ARGS)
{
	...
	if (!timestamptz_offset_internal_safe(timestamp, &tspan, &result, fcinfo->context))
		PG_RETURN_NULL();
...
timestamp_li_value_safe(float8 f, Timestamp y0, Timestamp y1, Timestamp *result, Node *escontext)
{
	...
	offset = DatumGetIntervalP(DirectFunctionCall2(interval_mul, IntervalPGetDatum(&diffy),
									Float8GetDatum(f)));
	if (!timestamp_offset_internal_safe(y0, offset, result, escontext)) {
		pfree(offset);
		return false;
	}
...
{
	/* Internally identical to Timestamp */
	return timestamptz_li_value_safe(f, y0, y1, result, escontext);
}
...
timestamp_zone(PG_FUNCTION_ARGS)
{
	...
	if (!DecodeTimezoneAbbrevSafe(0, lowzone, &val, &tzp, &type, fcinfo->context))
		PG_RETURN_NULL();
...
timestamp_timestamptz(PG_FUNCTION_ARGS)
{
	...
	if (!timestamp2timestamptz_safe(timestamp, &result, fcinfo->context))
		PG_RETURN_NULL();
